...
for (const project of PROJECTS) {
  assert.ok(project.title && project.description.en && project.description.es && project.proof.en && project.proof.es, `incomplete bilingual metadata: ${project.slug}`);
  assert.ok(['python', 'web', 'systems', 'data'].includes(project.category), `invalid category: ${project.slug}`);
